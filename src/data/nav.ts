// Single source of truth for the Notes sub-nav strip — every page in this
// standalone site is a Notes page (no primary bar / multi-section switch to
// maintain, unlike the source portfolio monorepo this was extracted from).
// Mirrors the pattern GITA_NAV_GROUPS established in the sibling gita repo.
...
// Carried over from the source portfolio monorepo's NOTES_NAV, minus
// `/flashcards` (zero coupling with notes — stays behind in that repo) and
// with `/search` re-pointed at `/notes/search` now that search lives inside
// this repo's own `/notes/` deployment base instead of the portfolio's
// top-level route.
